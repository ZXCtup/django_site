...
def new_comment_create(sender,instance,created, **kwarg):
    if created and instance.verify==False:
        send_message(f'''Новый сомнительный комментарий! ⛔

<b>id:</b> {instance.id} ⚙
                     
<b>От пользователя:</b> {instance.author.username} 👨‍💻

<b>На пост:</b> {instance.post.title} 🤖

<b>Текст комментария:</b>

<code>{instance.text}</code> 

<b>Дата публикации:</b> {instance.date} 🗓
